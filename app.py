...
def open_position(side, symbol, high, low, order_type=ORDER_TYPE_MARKET):  
    try:
        ...
        if check_main_order_status() != True:
            order = client.futures_create_order(symbol=symbol, side=side, type="TAKE_PROFIT",stopPrice=price, price=price, quantity=quantity, timeInForce=TIME_IN_FORCE_GTC,)
        else:
