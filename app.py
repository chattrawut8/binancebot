    ...
    try:
        ...
        if check_main_order_status(symbol) != True and check_position_status(symbol) != True:
            if side == "BUY":
                ...
                order = client.futures_create_order(symbol=symbol, side="SELL",  reduceOnly="true", closePosition="true",
                ...
                order = client.futures_create_order(symbol=symbol, side="BUY", closePosition="true",
                type="TAKE_PROFIT_MARKET",stopPrice=high_price, quantity=quantity, timeInForce=TIME_IN_FORCE_GTC,)
        else:
