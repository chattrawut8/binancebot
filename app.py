    ...
    try:
        ...
        if check_main_order_status(symbol) != True and check_position_status(symbol) != True:
            if side == "BUY":
                ...
                order = client.futures_create_order(symbol=symbol, side="SELL", closePosition="true",
                type="STOP_LOSS_MARKET",stopPrice=low_price, quantity=quantity, timeInForce=TIME_IN_FORCE_GTC,)
            else:
                order = client.futures_create_order(symbol=symbol, side=side, type="LIMIT",stopPrice=high_price, quantity=quantity, timeInForce=TIME_IN_FORCE_GTC,)
